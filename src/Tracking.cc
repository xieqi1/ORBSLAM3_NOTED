    ...
    mnInitialFrameId(0), mbCreatedMap(false), mnFirstFrameId(0), mpCamera2(nullptr)
{
    ...
    // Load ORB parameters
    bool b_parse_orb = ParseORBParamFile(fSettings);    // 创建orb特征提取对象
    if(!b_parse_orb)
    {
    ...
    {
        b_parse_imu = ParseIMUParamFile(fSettings); // 加载imu噪声相关参数
        if(!b_parse_imu)
        {
            std::cout << "*Error with the IMU parameters in the config file*" << std::endl;
        }

        mnFramesToResetIMU = mMaxFrames;    // 这个就是fps，就是一秒钟的时间
    }
...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp, string filename)
{
    ...
    mCurrentFrame.mnDataset = mnNumDataset; // 初始是0

    lastID = mCurrentFrame.mnId;    // frame的id
    Track();
...
void Tracking::PreintegrateIMU()
{
    ...
    if(!mCurrentFrame.mpPrevFrame)  // 必须是帧间
    {
    ...
    while(true) // 等到有足够的imu为止
    {
        bool bSleep = false;
        {
            ...
            if(!mlQueueImuData.empty())
            {
                ...
                {
                    mvImuFromLastFrame.push_back(*m);   // 多一个imu
                    break;
                }
            }
            else
    ...
    for(int i=0; i<n; i++)
    {
        ...
        if (!mpImuPreintegratedFromLastKF)  // 构造的时候就初始化了
            cout << "mpImuPreintegratedFromLastKF does not exist" << endl;
        // TODO 这里预积分和vins中不太像，需要细细的研究一下forster的论文，这里先放一放，作用应该和vins中一样
        // 到上一帧和上一关键帧分别积分累加
    ...
    mCurrentFrame.mpLastKeyFrame = mpLastKeyFrame;  // 绑定KF
...
void Tracking::Track()
{
    ...
    if(mpLocalMapper->mbBadImu) // 初始是false，TODO : 回头看一下这个操作
    {
    ...
    if(mState!=NO_IMAGES_YET)   // 初始状态就是没图片 这里检查一样时间戳
    {
        ...
        else if(mCurrentFrame.mTimeStamp>mLastFrame.mTimeStamp+1.0) // TODO 这个case回头再看
        {
    ...
    // 当前bias用上一个KF的bias做初始值，这个得初始化结束之后做
    if ((mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO) && mpLastKeyFrame)
        mCurrentFrame.SetNewBias(mpLastKeyFrame->GetImuBias());

    if(mState==NO_IMAGES_YET)
    {
        mState = NOT_INITIALIZED;   // 状态转换
    }

    mLastProcessedState=mState;
    // 带imu并且没有创建新的地图，注意创建新的地图意味着KF从头开始
    if ((mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO) && !mbCreatedMap)
    {
        PreintegrateIMU();  // imu积分 同时更新和上一帧以及上一KF之间预积分量
    }
    mbCreatedMap = false;   // 必然新的地图已经有KF了
    ...
    if(nCurMapChangeIndex>nMapChangeIndex)
    {
        ...
        pCurrentMap->SetLastMapChange(nCurMapChangeIndex);  // 地图改变id的更新，不知道要干啥
        mbMapUpdated = true;
    }


    if(mState==NOT_INITIALIZED) // 直接进来了
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            else
            {
                ...
                if (mState == RECENTLY_LOST)
                {
                    ...
                    {
                        if(pCurrentMap->isImuInitialized()) // 地图imu初始化好了
                            PredictStateIMU();
                        else
                            bOK = false;
                        // 等太久了，就更进糟糕的状态
...
void Tracking::MonocularInitialization()
{
    ...
    if(!mpInitializer)  // 没有初始化器说明第一次进来
    {
        // Set Reference Frame
        if(mCurrentFrame.mvKeys.size()>100)
        {
            ...
            for(size_t i=0; i<mCurrentFrame.mvKeysUn.size(); i++)
                mvbPrevMatched[i]=mCurrentFrame.mvKeysUn[i].pt; // 上一帧特征点的像素位置
            ...
            if (mSensor == System::IMU_MONOCULAR)
            {
                if(mpImuPreintegratedFromLastKF)    // 初始化都没好，不存在上一帧KF
                {
    ...
    else
    {
        ...
        if (((int)mCurrentFrame.mvKeys.size()<=100)||((mSensor == System::IMU_MONOCULAR)&&(mLastFrame.mTimeStamp-mInitialFrame.mTimeStamp>1.0)))
        {
            ...
            fill(mvIniMatches.begin(),mvIniMatches.end(),-1);   // 应该是empty，没啥用这里
        ...
        // 利用camera接口来进行重建 TODO 可以看看这里的相机模型
        if(mpCamera->ReconstructWithTwoViews(mInitialFrame.mvKeysUn,mCurrentFrame.mvKeysUn,mvIniMatches,Rcw,tcw,mvIniP3D,vbTriangulated))
        {
            ...
            {
                if(mvIniMatches[i]>=0 && !vbTriangulated[i])    // 有对应匹配点但是三角化失败了
                {
                    mvIniMatches[i]=-1;
                    nmatches--;
                }
            ...
            // Set Frame Poses
            mInitialFrame.SetPose(cv::Mat::eye(4,4,CV_32F));    // 第一帧位姿设置为单位阵
            cv::Mat Tcw = cv::Mat::eye(4,4,CV_32F);
            Rcw.copyTo(Tcw.rowRange(0,3).colRange(0,3));
            tcw.copyTo(Tcw.rowRange(0,3).col(3));
            mCurrentFrame.SetPose(Tcw); // 第二帧的位姿
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    if(mSensor == System::IMU_MONOCULAR)
        pKFini->mpImuPreintegrated = (IMU::Preintegrated*)(NULL);   // 第一帧不可以有预积分
    ...
    for(size_t i=0; i<mvIniMatches.size();i++)
    {
        ...
        //Create MapPoint.
        cv::Mat worldPos(mvIniP3D[i]);  // 已经三角化成功的3d点
        MapPoint* pMP = new MapPoint(worldPos,pKFcur,mpAtlas->GetCurrentMap());

        pKFini->AddMapPoint(pMP,i); // KF看到哪些地图点
        pKFcur->AddMapPoint(pMP,mvIniMatches[i]);

        pMP->AddObservation(pKFini,i);  // 地图点被哪些KF看到
        ...
        mCurrentFrame.mvpMapPoints[mvIniMatches[i]] = pMP;  // 当前帧（不是KF）地图点匹配对应的特征点id
...
void Tracking::UpdateLastFrame()
{
    ...
    mLastFrame.SetPose(Tlr*pRef->GetPose());    // 算出上一帧的世界坐标
...
bool Tracking::TrackWithMotionModel()
{
    ...
    {
        mCurrentFrame.SetPose(mVelocity*mLastFrame.mTcw);   // 匀速模型设置位姿
    }
