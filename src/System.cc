    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false)
{
    ...
    if (mSensor==IMU_STEREO || mSensor==IMU_MONOCULAR)
        mpAtlas->SetInertialSensor();   // 表示地图里带imu的
    ...
    mpLocalMapper->mInitFr = initFr;    // 这个是0
    mpLocalMapper->mThFarPoints = fsSettings["thFarPoints"];    // 远的扔掉的点
...
cv::Mat System::TrackMonocular(const cv::Mat &im, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    if (mSensor == System::IMU_MONOCULAR)
        for(size_t i_imu = 0; i_imu < vImuMeas.size(); i_imu++)
            mpTracker->GrabImuData(vImuMeas[i_imu]);    // 只扔imu数据暂时不做处理
